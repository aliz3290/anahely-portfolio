...
export default function Ages2To5BoyPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-200 via-red-200 to-blue-200 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-5xl font-bold mb-4 text-blue-800">🌟 Ages 2-5</h1>
          <p className="text-xl text-blue-700">Welcome to the preschool zone!</p>
        </div>
        ...
        <div className="bg-white/90 rounded-lg p-8 backdrop-blur-sm shadow-2xl mb-8">
          <div className="text-center space-y-6">
            ...
            <h2 className="text-3xl font-bold text-blue-800">Preschool Adventures</h2>
          ...
          {games.map((game) => (
            <div
              ...
              className={`bg-white/90 rounded-lg p-6 cursor-pointer transition-all hover:scale-105 shadow-lg ${
                selectedGame === game.id ? "ring-4 ring-blue-500" : ""
              }`}
            >
              <div className="text-center">
                <div className="text-5xl mb-3">{game.emoji}</div>
                <h3 className="font-bold text-blue-800 mb-2 text-xl">{game.title}</h3>
                <p className="text-blue-700 text-sm mb-3">{game.description}</p>
                {selectedGame === game.id && (
                  <div className="mt-4 p-4 bg-blue-100 rounded-lg">
                    <p className="text-blue-800 text-sm mb-3">{game.content}</p>
                    {game.id === "abc" && (
                      <div className="space-y-2">
                        <div className="text-6xl font-bold text-blue-600">{alphabet[currentLetter]}</div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                    ...
                    {game.id === "counting" && (
                      <div className="space-y-2">
                        <div className="text-6xl font-bold text-blue-600">{currentNumber + 1}</div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
        ...
        {selectedGame === "coloring" && (
          <div className="bg-white/90 rounded-lg p-8 backdrop-blur-sm shadow-2xl mb-8">
            <h3 className="text-2xl font-bold text-blue-800 mb-4 text-center">Choose Your Colors!</h3>
            <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
              {colors.map((color, index) => (
                <button
                  key={index}
                  className="p-4 bg-white rounded-lg border-2 border-blue-300 hover:border-blue-500 transition-all hover:scale-110 text-lg"
                >
                  {color}
                </button>
              ))}
