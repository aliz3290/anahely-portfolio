...
export default function Ages10To13GirlPage() {
...
rect(50, 50, 100, 100);`,
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-purple-200 via-pink-200 to-purple-200 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-5xl font-bold mb-4 text-purple-800">🎯 Ages 10-13</h1>
          <p className="text-xl text-purple-700">Welcome to the tween zone!</p>
        </div>

        <nav className="flex justify-center gap-4 mb-12">
          <Link
            href="/family-zone"
            className="px-6 py-3 bg-purple-500 hover:bg-purple-600 text-white rounded-lg transition-all font-semibold"
          >
            ← Back to Family Zone
          </Link>
          <Link
        ...
        <div className="bg-white/90 rounded-lg p-8 backdrop-blur-sm shadow-2xl mb-8">
          <div className="text-center space-y-6">
            ...
            <h2 className="text-3xl font-bold text-purple-800">Tween Activities</h2>
          ...
          {activities.map((activity) => (
            <div
              ...
              className={`bg-white/90 rounded-lg p-6 cursor-pointer transition-all hover:scale-105 shadow-lg ${
                selectedActivity === activity.id ? "ring-4 ring-purple-500" : ""
              }`}
            >
              <div className="text-center">
                <div className="text-5xl mb-3">{activity.emoji}</div>
                <h3 className="font-bold text-purple-800 mb-2 text-xl">{activity.title}</h3>
                <p className="text-purple-700 text-sm mb-3">{activity.description}</p>
                {selectedActivity === activity.id && (
                  <div className="mt-4 p-4 bg-purple-100 rounded-lg">
                    <p className="text-purple-800 text-sm mb-3">{activity.content}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {selectedActivity === "coding" && (
          <div className="bg-white/90 rounded-lg p-8 backdrop-blur-sm shadow-2xl mb-8">
            <h3 className="text-2xl font-bold text-purple-800 mb-4 text-center">💻 Learn to Code</h3>
            <div className="space-y-4">
              <div className="bg-gray-900 rounded-lg p-6 text-left">
                <div className="text-pink-400 text-sm mb-2">{codeSnippets[codeSnippet].language}</div>
                <pre className="text-green-400 text-sm font-mono">
                  {codeSnippets[codeSnippet].code}
                </pre>
              </div>
              <p className="text-purple-800 text-center">{codeSnippets[codeSnippet].description}</p>
        ...
        {selectedActivity === "quiz" && (
          <div className="bg-white/90 rounded-lg p-8 backdrop-blur-sm shadow-2xl mb-8">
            <h3 className="text-2xl font-bold text-purple-800 mb-4 text-center">🏆 Quiz Challenge</h3>
            <div className="space-y-4">
              <div className="text-xl font-bold text-purple-800 text-center mb-4">
              ...
              <div className="flex gap-4 justify-center">
                <button
                  ...
                  disabled={!quizAnswer}
                  className="px-6 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-lg font-bold disabled:opacity-50"
                >
                  Submit Answer
                </button>
                <button
                  ...
                  className="px-6 py-2 bg-pink-500 hover:bg-pink-600 text-white rounded-lg font-bold"
                >
                  Next Question
                </button>
              </div>
              {quizResult && (
                <div className="text-xl font-bold text-purple-800 text-center">{quizResult}</div>
              )}
            </div>
          </div>
        )}
        ...
        {selectedActivity === "creative" && (
          <div className="bg-white/90 rounded-lg p-8 backdrop-blur-sm shadow-2xl mb-8">
            <h3 className="text-2xl font-bold text-purple-800 mb-4 text-center">🎨 Creative Projects</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              {creativeProjects.map((project, index) => (
                ...
                >
                  <h4 className="font-bold text-purple-800">{project.name}</h4>
                </button>
              ))}
            </div>
            {creativeProject && (
              <div className="bg-purple-100 rounded-lg p-6">
                <h4 className="font-bold text-purple-800 mb-3 text-xl">{creativeProject}</h4>
